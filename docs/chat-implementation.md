...
- Resolve explicit calendar years, month ranges, ISO dates and relative periods. Earlier numeric baselines within two years may supplement the requested period and are explicitly labelled as outside it.
- Retain study findings and measurement endpoints across tests; do not fill the entire packet with one dense series. Genomics is included only when relevant. Wearable history is supplied as a small aggregate summary.
